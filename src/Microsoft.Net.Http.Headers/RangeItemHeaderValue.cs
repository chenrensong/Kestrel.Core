...
public class RangeItemHeaderValue
{
	...
	internal static int GetRangeItemListLength(StringSegment input, int startIndex, ICollection<RangeItemHeaderValue> rangeCollection)
	{
	...
	internal static int GetRangeItemLength(StringSegment input, int startIndex, out RangeItemHeaderValue parsedValue)
	{
		...
		long result = 0L;
		if (numberLength > 0 && !HeaderUtilities.TryParseNonNegativeInt64(input.Subsegment(offset, numberLength), out result))
		{
			return 0;
		}
		long result2 = 0L;
		if (num2 > 0 && !HeaderUtilities.TryParseNonNegativeInt64(input.Subsegment(offset2, num2), out result2))
		{
			return 0;
		}
